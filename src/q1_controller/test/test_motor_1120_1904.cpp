...
class LowlevelManager : public rclcpp::Node
{
  public:
    ...
    std::string ECClientUpdate2()
    {
        RCLCPP_INFO(this->get_logger(), "ECClientUpdate2.");
        // std::cout << "ECClientUpdate2" << std::endl;
        return motor_manager_->jointCommand(scaled_action * 0);
    }
    std::string ECClientUpdate()
    {
        ...
        else if (state == FSM_state::rl_run_state)
        {
            ...
            scaled_action = rl_control_->inference() * 0;
            // RCLCPP_INFO(this->get_logger(), "inference ok.");
            // std::cout << "scaled_action: " << scaled_action.size() << std::endl;
